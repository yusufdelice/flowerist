// Seed script - uses dynamic import to load the generated Prisma client
async function main() {
  // Dynamically import the generated client
  const { PrismaClient } = await import('../src/generated/prisma/client.js');
  const prisma = new PrismaClient();

  console.log('🌱 Seeding database...');

  // Clear existing data
  await prisma.product.deleteMany();
  await prisma.category.deleteMany();
  await prisma.banner.deleteMany();
  await prisma.siteSettings.deleteMany();

  // Create categories
  const categories = await Promise.all([
    prisma.category.create({ data: { name: 'Buketler', slug: 'buketler', sortOrder: 1 } }),
    prisma.category.create({ data: { name: 'Aranjmanlar', slug: 'aranjmanlar', sortOrder: 2 } }),
    prisma.category.create({ data: { name: 'Kutuda Çiçekler', slug: 'kutuda-cicekler', sortOrder: 3 } }),
    prisma.category.create({ data: { name: 'Düğün & Nişan', slug: 'dugun-nisan', sortOrder: 4 } }),
    prisma.category.create({ data: { name: 'Cenaze Çiçekleri', slug: 'cenaze-cicekleri', sortOrder: 5 } }),
    prisma.category.create({ data: { name: 'Saksı Çiçekleri', slug: 'saksi-cicekleri', sortOrder: 6 } }),
  ]);
  console.log(`✅ Created ${categories.length} categories`);

  // Create products
  const products = [
    { name: 'Kırmızı Gül Buketi', slug: 'kirmizi-gul-buketi', description: 'En kaliteli kırmızı güllerden oluşan muhteşem bir buket. 25 adet ithal kırmızı gül ile hazırlanmıştır. Sevdiklerinize aşkınızı en güzel şekilde ifade edin.\n\nBuket yüksekliği: 50 cm\nÇiçek sayısı: 25 adet\nAmbalaj: Özel tasarım kraft kağıt', price: 850, categoryId: categories[0].id, tags: 'romantik,sevgiliye,kırmızı,gül', featured: true },
    { name: 'Beyaz Papatya Buketi', slug: 'beyaz-papatya-buketi', description: 'Masumiyet ve saflığın simgesi beyaz papatyalardan oluşan zarif bir buket. Doğal yeşilliklerle desteklenmiştir.\n\nBuket yüksekliği: 40 cm\nÇiçek türü: Papatya, yeşillik\nAmbalaj: Keten kurdele ile bağlanmış', price: 420, categoryId: categories[0].id, tags: 'papatya,beyaz,zarif', featured: true },
    { name: 'Mevsim Çiçekleri Buketi', slug: 'mevsim-cicekleri-buketi', description: 'Mevsimin en taze ve renkli çiçeklerinden oluşan canlı bir buket.\n\nBuket yüksekliği: 45 cm\nÇiçek çeşidi: Mevsime göre değişir', price: 550, categoryId: categories[0].id, tags: 'mevsim,renkli,taze' },
    { name: 'Pembe Gül & Lilyum', slug: 'pembe-gul-lilyum', description: 'Zarif pembe güller ve kokulu lilyumlardan oluşan şık bir buket. Özel günler için ideal.\n\nBuket yüksekliği: 55 cm\nÇiçek sayısı: 15 gül, 3 lilyum', price: 750, categoryId: categories[0].id, tags: 'pembe,gül,lilyum,şık', featured: true },
    { name: 'Ayçiçeği Buketi', slug: 'aycicegi-buketi', description: 'Neşe ve mutluluk getiren parlak ayçiçeklerinden oluşan enerji dolu bir buket.\n\nBuket yüksekliği: 50 cm\nÇiçek sayısı: 10 adet', price: 480, categoryId: categories[0].id, tags: 'ayçiçeği,sarı,neşeli' },
    { name: 'Orkide Aranjmanı', slug: 'orkide-aranjmani', description: 'Beyaz seramik saksıda tek dallı beyaz orkide aranjmanı. Ofis ve ev dekorasyonu için ideal.\n\nYükseklik: 60 cm\nSaksı: Beyaz seramik', price: 650, categoryId: categories[1].id, tags: 'orkide,beyaz,dekoratif', featured: true },
    { name: 'Masa Aranjmanı', slug: 'masa-aranjmani', description: 'Renkli mevsim çiçeklerinden yuvarlak masa aranjmanı.\n\nÇap: 30 cm\nYükseklik: 25 cm\nKap: Cam vazo', price: 380, categoryId: categories[1].id, tags: 'masa,aranjman,renkli' },
    { name: 'Gül Aranjmanı', slug: 'gul-aranjmani', description: 'Kırmızı ve beyaz güllerin zarif uyumundan oluşan şık bir aranjman.\n\nYükseklik: 35 cm\nÇiçek sayısı: 30 adet karışık gül\nKap: Gold metal sepet', price: 920, categoryId: categories[1].id, tags: 'gül,aranjman,premium', featured: true },
    { name: 'Silindir Kutuda Güller', slug: 'silindir-kutuda-guller', description: 'Siyah silindir kutuda özenle yerleştirilmiş kırmızı güller. Lüks ve şık bir hediye.\n\nKutu çapı: 20 cm\nGül sayısı: 20 adet', price: 1100, categoryId: categories[2].id, tags: 'kutu,gül,lüks,hediye', featured: true },
    { name: 'Kalp Kutuda Çiçekler', slug: 'kalp-kutuda-cicekler', description: 'Kalp şeklinde özel kutuda kırmızı güller ve çikolatalar.\n\nKutu boyutu: 25x25 cm\nGül sayısı: 15 adet', price: 1350, categoryId: categories[2].id, tags: 'kalp,kutu,sevgililer,çikolata' },
    { name: 'Pastel Kutu Aranjmanı', slug: 'pastel-kutu-aranjmani', description: 'Pastel tonlarda çiçeklerle hazırlanmış zarif kutu aranjmanı.\n\nKutu boyutu: 18x18 cm\nÇiçek: Gül, karanfil, lisyantus', price: 780, categoryId: categories[2].id, tags: 'pastel,kutu,zarif' },
    { name: 'Gelin Buketi - Klasik', slug: 'gelin-buketi-klasik', description: 'Beyaz güller ve yeşilliklerle hazırlanan klasik gelin buketi.\n\nBuket çapı: 25 cm\nÇiçek: Beyaz gül, gipsofila', price: 1200, categoryId: categories[3].id, tags: 'gelin,düğün,beyaz,klasik' },
    { name: 'Gelin Buketi - Modern', slug: 'gelin-buketi-modern', description: 'Sukulent ve tropikal çiçeklerle modern bir gelin buketi.\n\nBuket stili: Asimetrik\nÇiçek: Protea, sukulent, okaliptüs', price: 1500, categoryId: categories[3].id, tags: 'gelin,modern,tropikal' },
    { name: 'Nişan Masası Süsleme', slug: 'nisan-masasi-susleme', description: 'Nişan masanız için komple çiçek süsleme paketi.\n\nKapsam: Masa ortası + 4 köşe aranjmanı\nNot: Detaylar için iletişime geçin.', price: 3500, categoryId: categories[3].id, tags: 'nişan,masa,süsleme' },
    { name: 'Taziye Çelengi', slug: 'taziye-celengi', description: 'Beyaz çiçeklerden hazırlanan saygı ifadesi taziye çelengi.\n\nBoyut: 80x120 cm\nÇiçek: Beyaz karanfil, lilyum, krizantem', price: 1800, categoryId: categories[4].id, tags: 'taziye,çelenk,cenaze' },
    { name: 'Taziye Aranjmanı', slug: 'taziye-aranjmani', description: 'Beyaz ve yeşil tonlarda hazırlanmış taziye aranjmanı.\n\nYükseklik: 40 cm\nKap: Hasır sepet', price: 680, categoryId: categories[4].id, tags: 'taziye,aranjman,beyaz' },
    { name: 'Bonsai Ağacı', slug: 'bonsai-agaci', description: 'İthal bonsai ağacı, dekoratif seramik saksıda.\n\nYükseklik: 35-40 cm\nSaksı: Kahverengi seramik', price: 950, categoryId: categories[5].id, tags: 'bonsai,saksı,dekoratif' },
    { name: 'Sukulent Terrarium', slug: 'sukulent-terrarium', description: 'Cam fanus içinde çeşitli sukulentlerden oluşan terrarium.\n\nÇap: 20 cm\nSukulent sayısı: 5-7 adet', price: 320, categoryId: categories[5].id, tags: 'sukulent,terrarium,kolay bakım' },
    { name: 'Lavanta Saksısı', slug: 'lavanta-saksisi', description: 'Mis kokulu lavanta bitkisi, dekoratif saksıda.\n\nYükseklik: 30 cm\nSaksı: Terracotta', price: 280, categoryId: categories[5].id, tags: 'lavanta,saksı,kokulu' },
    { name: 'Mor Orkide', slug: 'mor-orkide', description: 'Çift dallı mor orkide, şık beyaz seramik saksıda.\n\nYükseklik: 55 cm\nDal sayısı: 2', price: 780, categoryId: categories[5].id, tags: 'orkide,mor,saksı' },
  ];

  for (const product of products) {
    await prisma.product.create({
      data: {
        ...product,
        images: '[]',
        featured: product.featured || false,
        inStock: true,
        sortOrder: 0,
      },
    });
  }
  console.log(`✅ Created ${products.length} products`);

  // Create site settings
  await prisma.siteSettings.create({
    data: {
      id: 'main',
      phone: '+90 (216) 555 0123',
      whatsapp: '+902165550123',
      email: 'info@lotuscicekcilik.com',
      address: 'Bağdat Cad. No:123, Kadıköy/İstanbul',
      instagram: 'lotuscicekcilik',
      aboutText: 'Lotus Çiçekçilik, 2015 yılından bu yana İstanbul\'da hizmet vermektedir.',
      footerText: 'Her mevsim, her an, sevdiklerinize en özel çiçekleri hazırlıyoruz.',
    },
  });
  console.log('✅ Created site settings');

  console.log('🎉 Seed complete!');
  await prisma.$disconnect();
}

main().catch((e) => {
  console.error(e);
  process.exit(1);
});
